...
| 12   | Claude Design — reconcile the generated system, brief, screens. Subgates **12a–12e** in `FRONTEND-PLAN.md`              | 🟡 flow ran design→code, so this gate is a **reconciliation**. **12a** survey, **12b** token reconciliation, **12c** brief (`docs/DESIGN-BRIEF.md`) done — values landed, Gate 11 closed, two stale records corrected. Next: **12d** generate screens. Handed-back project is type `PROJECT_TYPE_PROJECT`, so 12d needs a new design-system project |

...
| 32   | **Chat: Markdown rendering** — assistant text parts render as real Markdown (today `**bold**` and `* bullets` show literally); also the renderer for prose inside gate 34's cards | ⬜ not started — spec as gate 31. One new frontend dep (Streamdown or `react-markdown`+`remark-gfm`, decide at impl per verify-docs). Frontend only. |

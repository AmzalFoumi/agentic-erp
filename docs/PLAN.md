...
| 30   | **Delivery discrepancy (F2)** — a dock worker types what actually arrived; the system works out the supplier credit | ⬜ design approved 2026-08-27, not built. The strongest AI story (turning "DairyCo sent 40 cases instead of 50, 2 broken" into structured numbers) and the most expensive, because it needs everything gate 29 builds plus credit memos and shrinkage. Last on purpose. One new table (`credit_memos`), no new permissions. Six things deliberately deferred rather than built (off-order arrivals, credit-against-future-orders, discounted-damaged-goods, multi-delivery receiving, overshipment, the `services/` package restructure) — recorded as alternatives, not gaps. Detail in `docs/FEATURES-PLAN.md` |

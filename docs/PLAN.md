...
| 26   | **Putting it online** — the website, the web API, the agent-server, the AI agent, **and the login server**, all hosted | ⬜ not started — blocked by gate 25, and this is a hard block, not a preference. **Note the count: five things to host, not four.** The login provider we chose has no hosted option, so we run it ourselves — that's a real cost of the choice, recorded in `AUTH-PLAN.md`. **The detail for this gate is in `docs/DEPLOY-PLAN.md`**, split out of `AUTH-PLAN.md` on 2026-08-26 because hosting is not a login question. How it gets deployed is still an open decision; the container recipe in `deploy/` is for local use only. **Three things gate 25 handed to this gate, all of which the machine-only lock currently covers and none of which survives that lock coming off:** (1) nothing rate-limits a caller who has not signed in; (2) the login server's certificate is still self-signed, and the two "don't check the certificate" switches (`NODE_TLS_REJECT_UNAUTHORIZED=0` on the website, `THUNDERID_VERIFY_TLS=false` in Python) must not survive to deployment; (3) **the agent-server decides who owns a conversation from a name it reads off the pass without checking the seal** — so once the agent-server is reachable from anywhere, someone could write their own pass and read another person's chat history. They could not *change* any data that way, because the MCP server does check seals — but reading is enough. Found by CodeRabbit on PR #30 and left open deliberately — the decision was made on 2026-08-26 and is written out in `DEPLOY-PLAN.md`, "Entry price: the agent-server's own door". In short: the agent-server must **verify** the pass it is handed (signature, expiry, issuer, a known audience) before reading a name off it, and it should eventually get an audience of its own. Neither half is done. |
...
**`docs/AUTH-PLAN.md`**, and gate 26 in **`docs/DEPLOY-PLAN.md`**.

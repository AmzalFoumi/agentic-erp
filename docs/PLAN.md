...
| 17   | Writing the agent's own conversation-handling code, with clear boundaries enforced automatically | ⬜ not started |
| 18   | Saving conversations to the database, with the same row-level security used elsewhere, and confirming this doesn't interfere with the backend's own database setup | ⬜ not started |

...
| 34   | **Chat: response cards** — a `tool-<name>` → React component registry renders the existing tool outputs as cards (product list, product, spoilage, lots, reorder bundles, pending drafts, purchase orders; generic fallback) instead of raw blobs; plus reload-persistence for tool parts and one `INSTRUCTIONS` line so the model stops re-typing list rows | ⬜ not started — spec as gate 31. Depends on 32 + 33. Hand-built cards from shadcn primitives + tokens, reusing `StockBadge` / `format.ts`. Reload-persistence touches `agent/conversation.py` `_completed` and the `Message` type, staying inside the gate-17 isolation cluster. **Rejected alternative** (dedicated "render" agent tools, the luxskin approach) and its ramifications are recorded in the spec. |

...
| 31   | **Chat: fullscreen / expand mode** — the agent panel can switch between the 256px right rail and filling the whole content area (top nav + side nav stay); the page is hidden, not unmounted, so **no route change, no refetch, nothing for ThunderID to re-evaluate** | ⬜ not started — design locked in `docs/superpowers/specs/2026-09-09-chatbot-overhaul-design.md`. Frontend only: one new `chat-shell.tsx` client wrapper in `layout.tsx`, `localStorage` mode like the density toggle. No backend, no auth, no routing. |

...
| 33   | **Chat: structured tool-output contract** — MCP read tools get `outputSchema` / Pydantic return models so tool `output` is real structured JSON (verified 2026-09-09: today it is a **string** of newline-concatenated JSON objects), plus generated TS types + a drift check like `schema.d.ts` | ✅ done 2026-09-10 (`e306d05..09b5714`) — MCP tools return named Pydantic models (`mcp_server/schemas.py`); `agent/mcp_client.py` prefers `structured_content`; `frontend/src/lib/api/mcp-schema.json` + `mcp-types.d.ts` generated + drift-checked in pytest. No `services/`/`authn/`/auth-path/ThunderID change; no new tools. |

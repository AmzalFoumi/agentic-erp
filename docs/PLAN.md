...
> | `docs/superpowers/specs/2026-09-09-chatbot-overhaul-design.md` | Gates 31–34: the chatbot overhaul — fullscreen/expand mode, Markdown rendering, structured tool-output contract, response cards. Includes the live wire-format capture from 2026-09-09 | Any chat-panel or agent-response work |
>
...
| 27   | **The approval queue** — the agent writes down what it wants to do; a manager reads it and presses a button | 🟨 code complete 2026-08-27, on `feat/erp/standout` — **not yet finished**, because the login server's own configuration is the half that is left. Built: the `action_drafts` table, the business logic, both front doors, and the `/approvals` screen. 132 backend and 34 agent tests pass. **The security shape is the point of this gate:** a draft names a *type* from a closed list, never a function, so a draft row can never become "run this code" — and the AI agent can propose but never approve, enforced in three separate places. What remains: the three new permissions have to be created on the login server, and the agent needs **its own role** first — the box currently gives the agent and the human the same one, so adding a human-only permission there would hand it to the agent with every test still passing. That finding is recorded in `docs/DEPLOY-PLAN.md`. Detail in `docs/FEATURES-PLAN.md` |
| 28   | **Spoilage and markdown (F1)** — find food about to go off, work out what to discount, propose it | 🟨 code complete 2026-08-27, on `feat/erp/standout`. `inventory_lots` landed and **migration `d5b93a17c204` is applied** — stock is no longer one number per product but batches with their own expiry dates and costs, and 22 backfill lots were created so no product's total disagrees with its lots. `products.quantity_on_hand` survives as a summary written in exactly **one** function, guarded by a source-level test. `adjust_stock` keeps its signature and now consumes soonest-expiry-first. New: `services/pricing.py` (the discount ladder, pure arithmetic), `services/lots.py`, `services/spoilage.py` (which registers the first real draft type, `BATCH_PRICE_MARKDOWN`), four HTTP routes, three MCP tools, and the `/inventory/spoilage` screen. **What remains: `lot.read` and `lot.write` do not exist on the login server yet**, so the feature 403s for a real token until they are created — plus the frontend type regeneration and a browser walkthrough. Detail in `docs/FEATURES-PLAN.md` |
| 29   | **Reorder and MOQ bundler (F3)** — group low stock by supplier, top the order up to the supplier's minimum | 🟨 code complete 2026-08-27, on `feat/erp/standout`. Migration `276428c7f1dd` (`create_purchasing_tables`) landed and is applied — four new tables (`suppliers`, `supplier_products`, `purchase_orders`, `purchase_order_lines`), all with RLS. New: `services/purchasing/` — the first package under `services/`, not a flat module, proving the shape before it is applied to older code — with `quantities.py` (pure arithmetic), the reorder bundler (`reorder.py`), the purchase-order state machine (`orders.py`), and `drafts.py` (the second real draft type, `SUPPLIER_REORDER`). Staging a bundle changes the `DraftHandler` contract everywhere: a handler now also receives the draft it came from, so `PurchaseOrder.source_draft_id` can be recorded without trusting an id inside an editable payload. Five suppliers and fourteen price-list links are seeded and verified against the live bundler — the preferred-tiebreak and top-up branches both fire for real, not just in tests. HTTP routes, three MCP tools (two read, one stages — no create/send/cancel tool reaches the agent), and the `/suppliers` and `/purchasing` screens are all in; frontend type-check, lint and production build are clean. **What remains: `purchasing.read` and `purchasing.write` do not exist on the login server yet**, so the feature 403s for a real token until they are created — batched with gate 28's `lot.read`/`lot.write` for after gate 30, per `docs/DEPLOY-PLAN.md`. Detail in `docs/FEATURES-PLAN.md` |
| 30   | **Delivery discrepancy (F2)** — a dock worker types what actually arrived; the system works out the supplier credit | 🟨 code complete 2026-08-28, on `feat/erp/standout`. The strongest AI story (turning "DairyCo sent 40 cases instead of 50, 2 broken" into structured numbers) and the most expensive, because it needs everything gate 29 builds plus credit memos and shrinkage. Migration `34334348fe8e` (`create_credit_memos_table`) landed and is applied. New: `services/purchasing/receiving.py` (the shared core both doors call), `propose_receipt` (the third real draft type, `DELIVERY_RECEIPT`), the direct-receive HTTP route, the `propose_delivery_receipt` MCP tool (the only door reachable from the agent — it only stages), and a receiving form embedded in the existing purchase-order detail page. **Verified, not assumed: no new ThunderID permission was introduced** — the shared core reuses `purchasing.write` and `lot.write`, both already defined and reused by gates 28–29, but — like those two — still pending on the login server, so this gate does not add an eighth or ninth item to that same wait. **The box needs nothing further for this gate** — `deploy/aisle-box/` points at the same shared Supabase database the `credit_memos` migration was already applied to, and `aisle_demo`'s DB role already has the grants it needs via the existing `alter default privileges`, so gate 30's box debt is genuinely zero. (An earlier version of this row warned the box was missing the table and would fail loudly on first use — that was wrong, corrected 2026-08-28.) Six things deliberately deferred rather than built (off-order arrivals, credit-against-future-orders, discounted-damaged-goods, multi-delivery receiving, overshipment, the `services/` package restructure), plus a seventh noted this session: an agent-suggested credit application, opt-in only, for a future gate. Detail in `docs/FEATURES-PLAN.md` |

| 31   | **Chat: fullscreen / expand mode** — the agent panel can switch between the 256px right rail and filling the whole content area (top nav + side nav stay); the page is hidden, not unmounted, so **no route change, no refetch, nothing for ThunderID to re-evaluate** | ✅ done 2026-09-09 (`5f7336f..42ae308`) — new `chat-shell.tsx` client wrapper owns the signed-in layout row; `layout.tsx` stays a Server Component. Mode (`docked`/`expanded`) read via `useSyncExternalStore` from `localStorage` key `agent-panel-mode` (density-toggle pattern, server snapshot `docked`, no hydration mismatch — verified in console). Expand/collapse icon button in the panel header, `Esc` collapses. Expanded = `<main>` hidden via `display:none` (still mounted, so client state, form inputs **and scroll position** survive collapse — confirmed in Chrome), panel `flex-1`, message column `max-w-3xl` centered. Walked in a browser against the dev stack: expand/collapse/Esc, streamed reply while expanded, reload-returns-expanded, signed-out landing has no panel — all pass. Frontend only; no backend, auth or routing touched. |
| 32   | **Chat: Markdown rendering** — assistant text parts render as real Markdown (today `**bold**` and `* bullets` show literally); also the renderer for prose inside gate 34's cards | ✅ done 2026-09-09 (`e4a73c2..186c50d`) — new `components/shell/agent-panel/markdown.tsx` wraps **Streamdown 2.6.0** (streaming-hardened `react-markdown` drop-in, verified from the published tarball — its `main` branch is a different API); `message-list.tsx` routes assistant text through it, user messages stay plain. `globals.css` gained `@import "streamdown/styles.css"` + a Tailwind v4 `@source` line. The hand-rolled blink cursor is replaced by Streamdown's own `caret="block"`. Images dropped (`disallowedElements`); `javascript:`/`<script>` neutralised by the built-in `rehype-sanitize`+`rehype-harden`; links keep `rel="noreferrer" target="_blank"` and are tinted via a `[&_a]:text-primary` token utility. Walked in a browser: bold/lists/GFM tables/inline code/code fences render, a half-streamed fence does not crash, dark mode + both densities follow, no console errors. Frontend only. |
| 33   | **Chat: structured tool-output contract** — MCP read tools get `outputSchema` / Pydantic return models so tool `output` is real structured JSON (verified 2026-09-09: today it is a **string** of newline-concatenated JSON objects), plus generated TS types + a drift check like `schema.d.ts` | ✅ done 2026-09-10 (`e306d05..72f8ded`) — MCP tools return named Pydantic models (`mcp_server/schemas.py`); `agent/mcp_client.py` prefers `structured_content`; `frontend/src/lib/api/mcp-schema.json` + `mcp-types.d.ts` generated + drift-checked in pytest. No `services/`/`authn/`/auth-path/ThunderID change; no new tools. |
| 34   | **Chat: response cards** — a `tool-<name>` → React component registry renders the existing tool outputs as cards (product list, product, spoilage, lots, reorder bundles, pending drafts, purchase orders; generic fallback) instead of raw blobs; one `INSTRUCTIONS` line so the model stops re-typing list rows | ✅ done 2026-09-10 (`88b04ae..809661f`) — **trimmed for a demo deadline** (spec split): card registry at `frontend/src/components/shell/agent-panel/cards/` (`tool-<name>` → component, unregistered tools → `FallbackCard`), `MessageList` rewritten to render every tool part inline (the old `if(!text)return null` early-return is gone), shared `ChatCard` shell + responsive `ChatCardTable` (real table expanded, stacked blocks in the 256px rail via `useChatMode()`), 7 typed read cards, `ToolCallCard`/`SuccessCard` re-parented onto the shell, one sentence appended to `agent/conversation.py`'s `INSTRUCTIONS`. Hand-built from tokens, reusing `StockBadge`/`format.ts`. No test runner on the frontend, so the gate is `tsc --noEmit` + `npm run lint` + typed fixtures in `cards/fixtures.ts` + a browser walkthrough (all 7 cards + approval/success + both panel modes verified against the dev stack; two findings fixed — `SuccessCard` `[object Object]` from gate 33's object output, and expanded-table header spacing). Agent side: 1 new pytest (`test_instructions.py`), full suite 41 pass. **Rejected alternative** (dedicated "render" agent tools, the luxskin approach) is recorded in the spec. |
| 34b  | **Chat: reload persistence + gate-33 cleanup** — whole-turn serialization in `agent/conversation.py` `_completed` so tool cards survive a reload (today only the text summary is restored), plus two parked gate-33 items (R3 `mcp_client` docstring rationale, R4 an unreachable fallback line) | ⬜ not started — deferred out of gate 34 for the demo deadline. Touches `agent/conversation.py` `_completed` and the `Message` type, staying inside the gate-17 isolation cluster. Detail in the spec's "Gate 34b" section. |
...
**`docs/AUTH-PLAN.md`**, gate 26 in **`docs/DEPLOY-PLAN.md`**, gates 27–30 in
**`docs/FEATURES-PLAN.md`**, and gates 31–34 in

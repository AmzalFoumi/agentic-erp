...
| `docs/AUTH-PLAN.md` | Gates 22–26: provider decision (ThunderID, pending a spike), RFC 8693 delegation, the ID-JAG-later rule, permission mapping | The auth gates |
...
| `frontend/AGENTS.md` (+ `frontend/CLAUDE.md`, which just includes it) | Generated by `create-next-app`. One instruction: read `node_modules/next/dist/docs/` rather than trusting training data about Next.js | Writing Next.js code. Framework-only — says nothing about this project, so it never overrides the five above |
...
same shape — `frontend/src/lib/auth/current-user.ts`, also hardcoded to `"system"`.
...
`SystemActor`. That's fine only because no unauthenticated caller exists yet — `agent/app.py` binds
